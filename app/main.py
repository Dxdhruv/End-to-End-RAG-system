...
def upload_document():
    try:
        ...
        try:
            ...
            logger.error(f"error uploading to the S3 bucket: {str(e)}")
            return jsonify({"error": f"Error uploading to the S3: {str(e)}"}), 500
        
        #Add to vector store
        try:
            vector_store.add_documents(text_chunks)
            logger.debug("Documents added to the vector store")
        except Exception as e:
            logger.debug(f"Error adding to the vector store: {str(e)}")
            return jsonify({"Error": f"Error uploading to the vector store: {str(e)}"}), 500
...
def query():
    ...
    if 'question' not in data:
        return jsonify({"error": f"No question provided"}), 400
    
    try:
        response = llm_service.get_response(data['question'])
        return jsonify({'response': response})
    except Exception as e:
        logger.debug(f"Some error occurred during sending query: {str(e)}")
        return jsonify({
            "error": str(e)
        }), 500
